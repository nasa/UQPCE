...
def user_function(user_func_str: str, X: np.ndarray):
    ...
    gen_res = func([X[:,i] for i in range(var_count)])

    return gen_res

def term_count(var_cnt, order):
    import math

    term_cnt = (
        math.factorial(var_cnt + order)
        /(math.factorial(var_cnt)*math.factorial(order))
    )
...
def solve_coeffs(var_basis, responses):
    ...
    # Consider replacing with OpenMDAO analysis error
