...
from uqpce.pce._helpers import solve_coeffs, warn
...
def calc_var_conf_int(matrix_coeffs, coeff_uncert, norm_sq):
    ...
    coeff_uncert = coeff_uncert.reshape(-1,)
...
def get_sobol_bounds(matrix_coeffs, sobols, coeff_uncert, norm_sq):
    ...
    coeff_uncert = coeff_uncert.reshape(matrix_coeffs.shape)
    ...
    if comm:
        ...
        lower_sobols_tot = lower_sobols
        upper_sobols_tot = upper_sobols

    return np.atleast_2d(lower_sobols_tot), np.atleast_2d(upper_sobols_tot)
...
def calc_error_sum_of_sq(var_basis, matrix_coeffs, responses):
    ...
    Design and Analysis of Experiments (8th) by Douglas Montgomery (pg. 453)
    """
    resp_cnt, coeff_cnt = var_basis.shape
    responses = responses.reshape(resp_cnt, -1)
    matrix_coeffs = matrix_coeffs.reshape(coeff_cnt, -1)

    resp_mat = np.matmul(responses.T, responses)
    coeff_dot_basis = np.matmul(matrix_coeffs.T, var_basis.T)

    err_sum_sq = resp_mat - np.matmul(coeff_dot_basis, responses)

    return err_sum_sq.ravel()
...
def calc_total_sum_of_sq(var_basis, responses):
    ...
    responses = responses.reshape(act_model_size, -1)
    resp_mat = np.matmul(responses.T, responses)

    tot_sum_sq = resp_mat - (np.sum(responses) ** 2 / act_model_size)

    return tot_sum_sq.ravel()
...
def calc_mean_sq_err(responses, matrix_coeffs, var_basis):
    ...
    if rank == 0:
        ...
        mean_sq_error = np.atleast_1d(np.array(mean_sq_error))
        mean_sq_error = mean_sq_error*np.identity(mean_sq_error.shape[0]) # vectorized for multiple responses

        # if value is negative, return the value that must be positive
        if (mean_sq_error < mean_sq_err_thresh).all():
