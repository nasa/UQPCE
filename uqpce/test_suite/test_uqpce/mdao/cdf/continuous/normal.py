...
if __name__ == '__main__':
    ...
    samples = dist.ppf(np.linspace(1e-3,1-1e-3,size))
    x = np.linspace(samples.min(), samples.max(), num=10000)
    guess = np.quantile(samples, p)

    prob = om.Problem()
    prob.model = CDFGroup(
        vec_size=size, alpha=sig, tanh_omega=omega, tail='upper', aleatory_cnt=size, 
        epistemic_cnt=1
    )
    ...
    f.suptitle('Normal Distribution', fontsize=tfs)
    f.legend(fontsize=12, bbox_to_anchor=(1, 1.1), fancybox=True, labelspacing=0.1)

    # plt.savefig('tanh_normal', bbox_inches='tight')