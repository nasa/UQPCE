...
from PCE_Codes.UQPCE import (
    ...
    GammaVariable, ProbabilityBoxes, UncertaintyType
)


class TestProbabilityBoxes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        unif_dict = {
            ...
            'interval_high':interval_high, 'type':'aleatory',
            'name':f'x{number}'
        }
        ...
        exp_dict = {
            ...
            'type':'aleatory', 'interval_low':interval_low,
            'name':f'x{number}'
        }
        ...
        beta_dict = {
            ...
            'type':'aleatory', 'interval_low':interval_low,
            'interval_high':interval_high, 'name':f'x{number}'
        }
        ...
        self.var_list_symb = self.pbox.generate_variable_str()
        self.pbox.count_epistemic()
        self.epist_var_count = self.pbox.epist_var_count
        self.pbox.generate_epistemic_samples(self.epist_samps, self.aleat_samps)
        self.pbox.generate_aleatory_samples()

        self.pbox.aleat_sub_samp_size = 5000
        self.pbox.epist_sub_samp_size = 25

        self.eval_resps, self.out_msg = self.pbox.evaluate_surrogate(
            self.var_basis_vect_symb, self.sig, self.resp_mean,
            self.matrix_coeffs, self.conv_thresh_percent
        )
        ...
        self.pbox.generate_epistemic_samples(
            self.epist_samps, self.aleat_samps
        )
        self.pbox.generate_aleatory_samples()

    def test_generate(self):
        """
        Testing ProbabilityBoxes generate for a system with 5 common
        variable types.
        ...
        self.conf_int_low, self.conf_int_high = (
            self.pbox.generate(self.eval_resps, self.sig, None)
        )
    ...
    def test_evaluate_surrogate(self):
        """
        Testing ProbabilityBoxes evaluate_surrogate for a system with 5
        ...
        self.assertTrue(
            (
                len(self.eval_resps[self.eval_resps != 0])
                >= 2 * self.pbox.aleat_sub_samp_size
            ),
            msg='ProbabilityBoxes evaluate_surrogate is not correct'
        )

    def test_generate_variable_str(self):
        """
        Testing ProbabilityBoxes generate_variable_str for a system 5 variables.
        """
        true_var_list_symb = [
            symbols('x0'), symbols('x1'), symbols('x2'), symbols('x3'),
            symbols('x4')
        ]

        self.assertEqual(
            true_var_list_symb, self.var_list_symb,
            msg='ProbabilityBoxes generate_variable_str is not correct'
        )

    def test_count_epistemic(self):
        """
        Testing ProbabilityBoxes count_epistemic for a system with 5 common
        variable types.
        """
    ...
    def test_generate_aleatory_samples(self):
        """
        Testing ProbabilityBoxes generate_aleatory_samples and ensuring that
        the correct number and type of samples are generated.
        """
        ...
        self.assertTrue(
            (unique_samp_count == self.pbox.epist_samps).all(),
            msg='ProbabilityBoxes generate_aleatory_samples is not correct'
        )

    def test_generate_epistemic_samples(self):
        """
        Testing ProbabilityBoxes generate_epistemic_samples and ensuring that
        the correct number and type of samples are generated.
        """
        unique_samp_count = np.array([
            len(np.unique(var.resample)) for var in self.var_list
        ...
        self.assertTrue(
            (
                unique_samp_count == (
                    self.pbox.epist_samps * self.pbox.aleat_samps
                )
            ).all(),
